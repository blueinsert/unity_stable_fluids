...
public class TestUI : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        int x = 0;
        int y = Screen.height - m_unit;
        GUI.Label(new Rect(x, y, m_unit * 10, m_unit * 2), $"FPS  {m_fps:f1} ({1000.0f / m_fps:f1})", m_textStyle);

        y -= m_unit;
        GUI.Label(new Rect(x, y, m_unit * 10, m_unit * 2), string.Format("output: {0}",FluidSimulation.Instance.CurrentOutputTextureInfo.m_desc), m_textStyle);
    }
